  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-8">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8 border-b pb-4 border-gray-100 gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 tracking-tight">My Sessions</h2>
          <p className="mt-1 text-gray-600">Track and manage your upcoming skill exchanges.</p>
        </div>
        ...
        <div className="flex space-x-1 p-1 bg-gray-100 rounded-lg">
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
              view === 'list' ? 'bg-white shadow text-blue-600' : 'text-gray-600 hover:bg-gray-200'
            }`}
          >
            List View
          </button>
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
              view === 'calendar' ? 'bg-white shadow text-blue-600' : 'text-gray-600 hover:bg-gray-200'
            }`}
          >
            Calendar
          </button>
      ...
      {sessions.length === 0 ? (
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 relative">
          ...
          {/* Event Popover */}
          {selectedEvent && (
            <div className="absolute inset-0 z-10 flex items-center justify-center p-4 bg-black bg-opacity-30" onClick={() => setSelectedEvent(null)}>
              <div 
              ...
              >
                ...
                <p className="font-medium text-gray-900 mb-2">Partner: {selectedEvent.partner?.name}</p>
                <p className="text-sm text-gray-600 mb-1">
                  <strong>Skill Exchanged:</strong> {selectedEvent.skillExchanged}
                </p>
                <p className="text-sm text-gray-600 mb-1">
                  <strong>Status:</strong> {selectedEvent.status}
                </p>
                <p className="text-sm text-gray-600">
                  <strong>Date:</strong> {new Date(selectedEvent.scheduledAt).toLocaleString()}
                </p>
              </div>
            </div>
          )}
